...
unsigned int smc_command;
unsigned int smc_param;       
...
// used by semi_loader.h to load the kernel (KVM) in normal world
struct loader_info loader;

// early uart driver before init of virtual memory
...
void load_pgtlb (uint32* kern_pgtbl, uint32* user_pgtbl)
{
	...
	// low vector tbl is important, then we can configure the base address ourself
	val |= 0x80100D; // enable MMU, cache, write buffer, low vector tbl,
					 // disable subpage
...
{
	// register smc handler function
	monitorInit(go_to_kernel);
}

void bootmain(void)
{
	// load kvm into memory, defined in semi_loader.h
	load_kernel(&loader);
	setup_hyp_vectors();
	...
	// change stack, the offset here is 0
    jump_stack ();
    
    // We can now call normal kernel functions at high memory
    //clear_bss ();
	
	early_printstr("[TZV]  Initialize STK...\n");

	// the mmu is enabled now
	kmain();	
}
